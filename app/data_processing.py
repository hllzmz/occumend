...
def load_and_prepare_data(config):
    """Loads data files, merges them, and performs clustering."""
    try:
        # Load and Merge Data
        df_jobs = pd.read_excel(
            config['OCCUPATIONS_FILE_PATH'], usecols=["O*NET-SOC Code", "Title"]
        ).set_index("O*NET-SOC Code")
        df_interests = pd.read_excel(config['INTERESTS_FILE_PATH'])
        ...
        riasec_mapping = {
            "1.B.1.a": "R_score", "1.B.1.b": "I_score", "1.B.1.c": "A_score",
            "1.B.1.d": "S_score", "1.B.1.e": "E_score", "1.B.1.f": "C_score",
        }
        ...
        df_job_profiles["cluster_name"] = df_job_profiles["cluster"].map(cluster_centers["cluster_name"].to_dict())
...
def _get_top_elements(filename, scale_id_filter="IM"):
    """Helper function: Gets the top 5 elements for each occupation."""
    try:
        df = pd.read_excel(filename)
        df_important = df[df["Scale ID"] == scale_id_filter]
        top_elements = df_important.sort_values("Data Value", ascending=False).groupby("O*NET-SOC Code").head(5)
        return top_elements.groupby("O*NET-SOC Code")["Element Name"].apply(list).to_dict()
    except FileNotFoundError:
        print(f"Warning: Competency file not found: '{filename}'. This section will be skipped.")
        return {}